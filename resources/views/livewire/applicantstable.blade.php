            ...
            <tbody>
                @foreach ($data as $key => $applicant)
                    <tr>
                        ...
                        <td>{{ $applicant->career?->company['cmpy'] }}</td>
                        <td>{{ $applicant->user['name'] }}</td>
                        <td>{{ $applicant->career['jobname'] }}</td>
                        <td>{{ $applicant->user?->profile['address'] }}</td>
                        <td><button type="button" class="btn btn-primary" data-toggle="modal"
                                data-target="#exampleModalCenter{{ $applicant->id }}">Show</button>
                            <div class="modal fade" id="exampleModalCenter{{ $applicant->id }}" tabindex="-1"
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <form action="{{ route('update-qrcode', ['id' => $applicant->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                        ...
                                        <form action="{{ route('applicants.destroy', $applicant->id) }}"
                                            ...
                                            <a class="dropdown-item"
                                                href="{{ route('employees.edit', $applicant->user['id']) }}">Edit
                                                User</a>
                                            <a class="dropdown-item"
                                                href="{{ route('applicants.show', $applicant->id) }}">Lihat Profil</a>
                                            @csrf
                                            @method('DELETE')
